...
public class Banana extends Fruta {
    ...
    public int tickRegen = 4;
    public double curaTotal = 4;
    public static String nome = "Banana";

    private transient BufferedImage[] spritesBanana;

    public Banana(int x, int y, int width, int height, BufferedImage sprite) {
        super(x, y, width, height, sprite, nome);
        spritesBanana = new BufferedImage[qtdDirecoes];
        for (int i = 0; i < qtdDirecoes; i++) {
            spritesBanana[i] = Game.spritesheet_Fruits.getSprite(64 * i, 64 * 7, 64, 64);
        }
        this.sprite = spritesBanana[0];

    }

    public Banana(Banana outroBanana) {
        super(outroBanana);
    ...
    public void render(Graphics g) {
        g.drawImage(spritesBanana[index], this.getX() - Camera.x, this.getY() - Camera.y, 32, 32, null);
    }
